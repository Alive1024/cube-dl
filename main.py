...
def ls(args: argparse.Namespace):
    ...
    def _draw_nested_table(rich_table, items, inner_table_key,
                           prompt_on_empty, width_ratios=(8, 12, 20, 10, 50)):
        if len(items) > 0:
    ...
    if args.projs_exps:
        ...
        _draw_nested_table(table, projects_exps, inner_table_key="Exps",
    ...
    # Print all projects
        ...
        _draw_table(table, proj_dao.get_projects(OUTPUT_DIR),
        ...
        _draw_nested_table(table, exps_runs, inner_table_key="Runs",
    ...
    # Print all exps of the specified proj
        ...
        _draw_table(table, exp_dao.get_exps_of(OUTPUT_DIR, proj_id=args.exps_of),
    ...
    # Print all runs of the specified exp of the specified proj
        ...
        table = get_table(title=f"All Runs of exp \"{args.runs_of[1]}\" of "
                                f"proj \"{args.runs_of[0]}\" in \"{OUTPUT_DIR}\"")
        _draw_table(table, run_dao.get_runs_of(OUTPUT_DIR, proj_id=args.runs_of[0], exp_id=args.runs_of[1]),
...
def _exec(args: argparse.Namespace, job_type: RootConfig.JOB_TYPES_T):
    ...
    # For any other run, a new one should be created.
        ...
        run = EntityFactory.get_run_instance(name=args.name, desc=args.desc, output_dir=OUTPUT_DIR,
                                             proj_id=args.proj_id, exp_id=args.exp_id, job_type=job_type)
